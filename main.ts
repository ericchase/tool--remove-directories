...
Deno.stdout.writeSync(new TextEncoder()
    .encode('\nDirectories found: 0'));

const paths: string[] = [];
const callback = function (path: string) {
    Deno.stdout.writeSync(new TextEncoder()
        .encode(`${'\b'.repeat(paths.length.toString().length)}${paths.length + 1}`));
    paths.push(path);
}
await searchDirectories('.', args['_'] as string[], args['--'] ?? [], callback);
